...
/**
 * Esta variável recebe por METHOD POST o nome do usuario para que o mesmo possa ser atualizado.
 * @access public
 * @name $nome
 */
$nome = isset($_POST['nome'])?$_POST['nome']:'';
/**
 * Esta variável recebe por METHOD POST o email do usuario para que o mesmo possa ser atualizado.
 * @access public
 * @name $email
 */
$email = isset($_POST['email'])?$_POST['email']:'';
/**
 * Esta variável recebe por METHOD POST a senha do usuario para que a mesma possa ser atualizada.
 * @access public
 * @name $senha
 */
$senha = isset($_POST['senha'])?$_POST['senha']:'';
/**
 * Esta variável recebe por METHOD POST o numero de telefone celular do usuario para que o mesmo possa ser atualizado.
 * @access public
 * @name $celular
 */
...
if($post === md5('seg'.$_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'])) {
    /**
     * se o conteudo da variavel $excluir for igual a 'excluir', sera passado o $id do usuario para o sistema realizar a exclusão
     */
    if ($excluir === 'excluir') {
        $drop = new \Root\Delete();
        $drop->deleteTableUsuario($id);
     ...
     */
        ...
        $update->updateTableUsuarios($id, $nome, $email, $senha, $celular);
    }
}
/**
