...
/**
 * Esta variável recebe por METHOD POST o nome do usuario a ser inserido no sistema.
 * @access public
 * @name $nome
 */
$nome = isset($_POST['nome'])?$_POST['nome']:'';
/**
 * Esta variável recebe por METHOD POST o email do usuario a ser inserido no sistema.
 * @access public
 * @name $email
 */
$email = isset($_POST['email'])?$_POST['email']:'';
/**
 * Esta variável recebe por METHOD POST a senha do usuario a ser inserido no sistema.
 * @access public
 * @name $senha
 */
 ...
 * que usuário acessa o formulario de cadastro de usuario, sendo assim possivel validar a origem dos dados e evitar
...
if($post === md5('seg'.$_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'])){
    ...
    if(!empty($nome) && !empty($email) && !empty($senha) && !empty($celular)) {
        ...
        $insert->insertTableUsuario($nome, $email, $senha, $celular,"../usuarios");
    }
    /**
