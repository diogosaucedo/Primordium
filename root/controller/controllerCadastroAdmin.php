...
/**
 * Esta variável recebe por METHOD POST o nome do administrador a ser inserido no sistema.
 * @access public
 * @name $nome
 */
$nome = isset($_POST["nome"])?$_POST["nome"]:'';
/**
 * Esta variável recebe por METHOD POST o email do administrador a ser inserido no sistema.
 * @access public
 * @name $email
 */
$email = isset($_POST["email"])?$_POST["email"]:'';
/**
 * Esta variável recebe por METHOD POST a senha do administrador a ser inserido no sistema.
 * @access public
 * @name $senha
 */
$senha = isset($_POST["senha"])?$_POST["senha"]:'';
/**
 * Esta variável recebe por METHOD POST o numero de telefone celular do administrador a ser inserido no sistema.
 * @access public
 * @name $celular
 */
 ...
 * que usuário acessa o formulario de cadastro de administradores, sendo assim possivel validar a origem dos dados e evitar
...
if($post === md5('seg'.$_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'])){
    /**
     * As variaveis $nome, $email, $senha e $celular não podem estar vazias.
     */
    if(!empty($nome) && !empty($email) && !empty($senha) && !empty($celular)){
        $create = new \Root\Create();
        $create->insertTableAdmin($nome,$email,$senha,$celular);
    }
    /**
